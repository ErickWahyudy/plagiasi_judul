...
class Login extends CI_controller
{
    ...
    public function index()
    {
        if (isset($_POST['login'])) {
            ...
            $nidn = $this->input->post('email');
            $email = $this->input->post('email');
            $password = $this->input->post('password');

            // Cek data login
            $superadmin = $this->Login_m->Superadmin($nama, $nidn, $email, md5($password));
            $admin = $this->Login_m->Admin($nama, $nidn, $email, md5($password));
            $dosen = $this->Login_m->Dosen($nama, $nidn, $email, md5($password));
            ...
            if ($superadmin->num_rows() > 0) {
                ...
                if ($isEmailValid->num_rows() > 0) {
                    ...
                    $pesan = '<script>
                    swal({
                        title: "NIDN / Email Salah atau Tidak Terdaftar",
                        type: "error",
                        showConfirmButton: true,
                        confirmButtonText: "OKEE"
                    });
